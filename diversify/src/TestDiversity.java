...
public class TestDiversity {
	 ...
	 * Update 2/25/2017
	 *
	 * Change args to database credential
	 *
	 *
	 */
	public static void main(String[] args) throws FileNotFoundException {
		...
		**/
		...
		int ar = 2;
		query = args[0];
		ps = new PrintStream(new FileOutputStream(args[1] + "/" + CleanString(args[0]) + ".txt"));
		while(!args[2].equals(""))
		{
			article_ids.add(args[ar]);
			ar++;
		}
		...
		//copy paste article_ids
		HashMap<String, String> docs = GetDocs_sql(article_ids);
...
//				PLSR_kernel /* div */ ));
	...
	/*getdoc from database*/
	public static HashMap<String, String> GetDocs_sql(List<String> articleid){

		HashMap<String, String> docs = new HashMap<String, String>();

		for (String art_id : articleid) {
			String content = getAbstractOfPapers(art_id);
			if (content == null)
				System.out.println("Could not read content for: " + art_id + "... skipping");
			else {
				docs.put(art_id, content);
			}
		}
		return docs;
	}

	public String getAbstractOfPapers(String art_id)
	{
		//List<Paper> paperList = new ArrayList<Paper>();

		String query = String.format("select abstract from paper_abstract where article_id = ", art_id,";");
		...
		try{
			...
			if (rs.next()){
				...
				System.out.print("No abstract for this article id found");
			}
