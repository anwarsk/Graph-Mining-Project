...
public class CSVAccessLayer {
	...
	public void readFeatureGeneratorProcessedLog(String processedLogFilePath, FeatureGeneratorInput featureGeneratorInput)
	{
		assert processedLogFilePath != null : "Null input file path";
		assert processedLogFilePath != "" : "Empty input file path";
		
		String line = "";

		try (BufferedReader buffReader = new BufferedReader(new FileReader(processedLogFilePath))) 
		{
			buffReader.readLine();
			...
			while ((line = buffReader.readLine()) != null) {
				...
				featureGeneratorInput.addProcessedInput(authorId, proceedingId);
	...
	public void readFeatureGeneratorInput(String inputFilePath, FeatureGeneratorInput featureGeneratorInput)
	{
		assert inputFilePath != null : "Null input file path";
		assert inputFilePath != "" : "Empty input file path";
	...
	public synchronized void writeFeatureGeneratorOutput(String outputFilePath, FeatureGeneratorOutput featureGeneratorOutput)
	{
	...
	public synchronized void writeFeatureGeneratorProcessLog(String featureGeneratorProcessLogFilePath, String authorId2,
															int proceedingId2) {
		
		assert featureGeneratorProcessLogFilePath != null : "Null output file path";
		assert featureGeneratorProcessLogFilePath != "" : "Empty output file path";

		PrintWriter fileWriter;
		try {
			File outputFile = new File(featureGeneratorProcessLogFilePath);
			boolean isOutputFileExists = outputFile.exists();
			fileWriter = new PrintWriter(new BufferedWriter(new FileWriter(outputFile, true)));
			if(!isOutputFileExists)
			{
				outputFile.createNewFile();
				fileWriter.write("AuthorId,ProceedingId\n");
			}
			fileWriter.println(authorId2 + "," + proceedingId2);
			fileWriter.flush();
			fileWriter.close();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
